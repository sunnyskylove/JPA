...
public class EmbeddedIdTest {
    ...
    /* 필기.
    *   복합키가 존재하는 테이블의 매핑
    *   1. EmbeddedId : @Embeddable 클래스에 복합키를 정의하고 엔티티에 @EmbeddedId 를 이용하여 복합키 클래스를 매핑한다.
    ...
    *   @EmbededId가 조금 더 객체 지향적인 방식이고, @IdClass 는 관계형 데이터베이스에 가까운 방법이다.
    * */

    private static Stream<Arguments> getLikeInfo() {
        return Stream.of(                                   // 아래 4가지의 경우의 수를 전달해 본다!
                Arguments.of(1,1),
                Arguments.of(2,1),
                Arguments.of(1,2),
                Arguments.of(2,2)
