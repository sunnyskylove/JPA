...
public class GroupTests {
    ...
    *   -> 따라서 반환 값을 담기 위해 선언하는 변수 타입을 기본자료형으로 하면 조회 결과를 언박싱 할 때 nullpointerexception 이 발생한다.
    *   3. 그룹함수의 반환 자료형은 Long or Double 이기 때문에 Having 절에서 그룹 함수 결과 값과 비교하기 위한 파라미터 타입은 Long 또는 Double 로 해야한다.
    * */

    /* 필기.
    *   # Group By -> 데이터를 특정 칼럼을 기준으로 그룹화 한다.
    *   select category_code, count(*) from tbl_menu 카테코드를 기준으로 그룹화 한다.
    ...
    * */
    ...
    void testCountMenuOfCategory() {
        ...
        int categoryCode = 4;       // 행이 몇개인지 세고 싶을 떄 사용
    ...
    void testWithoutCount() {
        ...
        int categoryCode = 50;          // 실제 데이터베이스에서 없는 존재 (50) 일떄, 테스트 해보기~!!

//        long sumOfPrice = repository.noResult(categoryCode);    // 50이라는 조회 결과 없을때~~
...
//        int menuPrice = 50000;
        long minPrice = 50000L;        // Long Type 은 뒤에 L 을 붙여준다.
